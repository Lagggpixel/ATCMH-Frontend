...
export default function SiteFrame({children, showLogin = false, footer = true, accountAccessory}: {children: ReactNode; showLogin?: boolean; footer?: boolean; accountAccessory?: ReactNode}) {
  return <div className="marketing-product unified-product"><SiteHeader variant="solid" showLogin={showLogin} accountAccessory={accountAccessory}/><div className="unified-product-content">{children}</div>{footer ? <SiteFooter/> : null}</div>;
}
