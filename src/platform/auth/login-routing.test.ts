...
test("direct home login uses the Dashboard audience and returns home", () => {
  assert.deepEqual(resolveHomeLoginRequest(new URLSearchParams()), {returnTo: "/"});
});
...
test("contextual login rejects cross-application and malicious destinations", () => {
  ...
  assert.equal(resolveHomeLoginRequest(new URLSearchParams({loginFor: "exams", returnTo: "/dashboard"})).returnTo, "/dashboard");
});
...
test("compatibility links preserve only supported auth errors", () => {
  assert.equal(homeLoginHref("dashboard", "/account", "consent_expired"), "/?login=1&returnTo=%2Faccount&authError=consent_expired");
  assert.equal(homeLoginHref("dashboard", "/account", "stack trace <script>"), "/?login=1&returnTo=%2Faccount");
});
...
test("invalid login errors preserve only an opaque support reference", () => {
  ...
  assert.equal(homeLoginHref("dashboard", "/account", "invalid_login", "6a82cb40-7a5e-4fb6-a3ca-91ec5f1258a1"), "/?login=1&returnTo=%2Faccount&authError=invalid_login&authRef=6a82cb40-7a5e-4fb6-a3ca-91ec5f1258a1");
  assert.equal(homeLoginHref("dashboard", "/account", "stack trace", "6a82cb40-7a5e-4fb6-a3ca-91ec5f1258a1"), "/?login=1&returnTo=%2Faccount");
});
