...
export default function CourseCenter({users, token, view, courseId}: CourseCenterProps) {
    ...
    return <section className={styles.center} aria-labelledby={headingId}>
        ...
        {courses && data && canAccessView && view === "course-create" ? <CourseEditor course={null} quizzes={data.quizzes} activities={[]} token={token} canPublish={data.actor.canManageAll || data.actor.capabilities.includes("publish-exams")} onCancel={showCatalog} onSaved={saved}/> : null}
        {courses && data && canAccessView && view === "course-edit" ? course ? <CourseEditor course={course} quizzes={data.quizzes} activities={course.activities ?? []} token={token} canPublish={data.actor.canManageAll || data.actor.capabilities.includes("publish-exams")} onCancel={showCatalog} onPreview={() => navigate(`/dashboard/courses/${course.id}/preview`)} onSaved={saved}/> : <p className={styles.state} aria-live="polite">Loading course editor…</p> : null}
