...
export function RescanStatus({scan}: { scan: AltEvidenceScan | null }) {
    if (!scan) return null;
    if (scan.state === "RUNNING") return <p className={styles.scanStatus} role="status">Rescanning
        account {scan.accountId}: {scan.completed} of {scan.total} addresses
        checked{scan.failed ? ` · ${scan.failed} unavailable` : ""}.</p>;
    if (scan.state === "FAILED") return <p className={styles.error} role="alert">Rescan failed
        ({scan.failureCode ?? "RESCAN_FAILED"}). Existing evidence is unchanged.</p>;
    return <p className={styles.scanStatus} role="status">Rescan
        complete: {scan.completed} checked, {scan.failed} unavailable.{scan.truncated ? " The 100 most recent addresses were refreshed; older stored evidence remains visible." : ""}</p>;
}

export function AltActionDialog({pending, reason, dialogRef, onReason, onSubmit, onCancel}: {
    ...
    return pending ? <div className={styles.modalBackdrop} role="presentation">
        <form ref={dialogRef} className={styles.modal} onSubmit={onSubmit} role="dialog" aria-modal="true"
              aria-labelledby="suppression-title"><p className={styles.eyebrow}>Evidence suppression</p><h2
            id="suppression-title">{pending.kind === "reverse" ? "Reverse suppression" : pending.kind === "vpn" ? "Classify as VPN" : `Detach account ${pending.accountId}`}</h2>
            <p>This changes candidate matching only. It does not delete security events or make an account decision.</p>
            <label>Required reason<textarea autoFocus required maxLength={512} value={reason}
                                            onChange={event => onReason(event.target.value)}/></label>
            <div>
                <button type="submit" disabled={!reason.trim()}>Confirm</button>
...
export function AltEvidenceActions({candidate, onAction}: {
...
function AccountLinks({accounts, onSelect}: { accounts: string[]; onSelect: (accountId: string) => void }) {
    return <div className={styles.accounts}>{accounts.map(account => <span key={account}><Link
        to={`/dashboard/accounts?accountId=${encodeURIComponent(account)}`}>Account {account}</Link><button
        type="button" onClick={() => onSelect(account)}>Use as subject</button></span>)}</div>;
}

function CandidateCard({candidate, onSelect, onAction}: {
    candidate: AltAccountCandidate;
...
const sectionCopy: Record<EvidenceType, { title: string; description: string }> = {
    SAME_IP: {
        title: "Exact same-IP groups",
        description: "Accounts grouped only when their canonical recorded address is exactly equal."
    },
    VPN_INDICATOR: {
        title: "VPN-related indicators",
        description: "Manual classifications and normalized results from the provider already configured by ATCMH."
    },
    NETWORK_SIMILARITY: {
        title: "Similar network candidates",
        description: "Distinct addresses grouped by IPv4 /24 or IPv6 /64 prefix so nearby candidates stay together."
    },
    OWNERSHIP_CONFLICT: {
        title: "Identity conflicts",
        description: "Historical conflicts from canonical account-link records."
    },
};

export function EvidenceSections({candidates, onSelect, onAction}: {
    candidates: AltAccountCandidate[];
    onSelect: (accountId: string) => void;
    onAction: (action: PendingAction, trigger: HTMLButtonElement) => void
}) {
...
export default function AdminAltAccounts({csrfToken, adminUser, loaded}: {
    ...
    const load = useCallback(async (accountId?: string) => {
        ...
        setError(null);
        try {
            const result = await ApiUtils.getAltAccounts(csrfToken, accountId);
            if (mounted.current) {
    ...
    useEffect(() => {
        if (adminUser?.canReviewAltAccounts) void load();
    }, [adminUser?.canReviewAltAccounts, load]);
    useEffect(() => {
        if (!pending) return;
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (result.focusIndex != null) {
                event.preventDefault();
                controls[result.focusIndex]?.focus();
            }
        };
    ...
    const selectSubject = (accountId: string) => {
        setSubjectInput(accountId);
        setSelectedAccountId(accountId);
        setScan(null);
        void load(accountId);
    };
    const reviewSubject = (event: FormEvent) => {
        event.preventDefault();
        const value = subjectInput.trim();
        if (!/^\d+$/.test(value)) {
            setError("Enter a canonical numeric account ID.");
            return;
        }
        selectSubject(value);
    };
    ...
    const beginRescan = async () => {
        ...
        try {
            ...
            if (mounted.current) setStartingScan(false);
        }
    };
    const openPending = (action: PendingAction, trigger: HTMLButtonElement) => {
    ...
    const submit = async (event: FormEvent) => {
        event.preventDefault();
        if (!pending || !csrfToken || !reason.trim()) return;
        try {
    ...
    return <main className={styles.page}>
        <section className={styles.subjectPanel}>
            <form onSubmit={reviewSubject}><label>Subject account ID<input inputMode="numeric" pattern="[0-9]+"
                                                                           value={subjectInput}
                                                                           onChange={event => setSubjectInput(event.target.value)}
                                                                           placeholder="Canonical account ID"/></label>
                <button type="submit">Review subject</button>
                <button type="button" onClick={showAll}>Show all</button>
            </form>
            <div>
                <strong>{selectedAccountId ? `Reviewing account ${selectedAccountId}` : "All current evidence groups"}</strong>
                <button type="button" onClick={() => void beginRescan()}
                        disabled={!selectedAccountId || startingScan || scan?.state === "RUNNING"}>{startingScan || scan?.state === "RUNNING" ? "Rescanning…" : "Rescan selected subject"}</button>
            </div>
            <RescanStatus scan={scan}/></section>
        {error ? <p className={styles.error} role="alert">{error}</p> : null}
        {loading ? <p>Loading evidence…</p> : candidates.length ?
            <EvidenceSections candidates={candidates} onSelect={selectSubject} onAction={openPending}/> :
        ...
        <section className={styles.suppressions}><h2>Suppression history</h2><p>Original audit and login evidence
            ...
            <em>Reversed</em> : <button type="button" onClick={event => openPending({
                kind: "reverse",
                suppression: item
            }, event.currentTarget)}>Reverse</button>}</article>)}</section>
        <AltActionDialog pending={pending} reason={reason} dialogRef={dialogRef} onReason={setReason} onSubmit={submit}
