...
const createEnvironment = (confirmResult = true) => {
    ...
    const location = {
        href: "https://dashboard.atcmh.org/dashboard/exams/new",
        origin: "https://dashboard.atcmh.org",
        assigned: [] as string[],
        assign(url: string) {
...
test("dirty protection captures same-origin links and removes every listener when disarmed", () => {
    ...
    const click = {
        ...
        target: {
            closest: () => ({href: "https://dashboard.atcmh.org/dashboard/exams"}),
        },
    };
    ...
    assert.deepEqual(environment.location.assigned, ["https://dashboard.atcmh.org/dashboard/exams"]);
