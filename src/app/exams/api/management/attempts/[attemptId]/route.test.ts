...
function authorize(roles: string[]) {
  ...
  globalThis.fetch = async (input) => String(input).includes("/internal/auth/sessions/introspect")
    ...
    : Response.json({
      capabilities: roles.includes("admin-role") ? ["review-attempts", "manage-system"] : roles.includes("mentor-role") ? ["review-attempts"] : [],
      canManageAll: roles.includes("admin-role"),
    });
}

function request(method: "GET" | "DELETE") {
  return new Request(`https://www.atcmh.org/exams/api/management/exams/attempts/${attemptId}`, {
    method,
    headers: { cookie: `__Host-atcmh_session=${token}`, origin: "https://www.atcmh.org", "X-CSRF-Token": csrfTokenFor(token) },
  });
}
