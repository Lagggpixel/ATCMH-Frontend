...
function authorizeMentor() {
  ...
  globalThis.fetch = async (input) => {
    ...
    return Response.json({ capabilities: ["manage-exams"], canManageAll: false });
  };
}
...
function request(method: "GET" | "PUT", body?: string) {
  return new Request(`https://www.atcmh.org/exams/api/management/exams/quizzes/${quizId}`, {
    method,
    headers: { cookie: `__Host-atcmh_session=${token}`, origin: "https://www.atcmh.org", "X-CSRF-Token": csrfTokenFor(token), ...(body ? { "Content-Type": "application/json" } : {}) },
    body,
  });
}
