...
test("RootSite marketing and legal content is owned by App Router routes", () => {
  for (const route of ["app/page.tsx", "app/terms/page.tsx", "app/policy/page.tsx"]) {
  ...
  const policy = readFileSync(new URL("../app/policy/page.tsx", import.meta.url), "utf8");
...
test("the unified app intentionally has no apply route", () => {
  assert.equal(existsSync(new URL("../app/apply", import.meta.url)), false);
});
